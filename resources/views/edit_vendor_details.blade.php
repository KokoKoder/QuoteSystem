...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
	...
	$sql="UPDATE vendor_address SET company_name='$company_name', address='$address', email='$email', phone='$phone', rg_kood='$rg_kood', konto='$konto', eu_vat_nb='$vat_nb' WHERE vendor_id='$vendor_id'";
...
if (isset($_GET['vendor_id'])) {
	...
	$konto=mysqli_real_escape_string($conn,$_POST["konto"]);
	$vat_nb=mysqli_real_escape_string($conn,$_POST["vat_nb"]);
	$sql="SELECT company_name, address, email, phone, rg_kood, konto, eu_vat_nb WHERE vendor_id='$vendor_id' FROM vendor_address WHERE vendor_address.vendor_id='$vendor_id'";
	if ($conn->query($sql) === TRUE) {
    echo "Record updated successfully";
	} else {
		echo "Error updating record: " . $conn->error;
...
@section('content')
  ...
  <div class="row">
	...
	<div class="row">
		  ...
		  <i class="material-icons prefix">profile</i>
