...
if (isset($_SESSION['vendor_id'])){
	...
	$sql="SELECT vendor_name,company_name,address,email,phone,rg_kood,konto,eu_vat_nb FROM vendor JOIN vendor_address ON vendor_address.vendor_id=vendor.vendor_id WHERE vendor.vendor_id='$v_id'";
	$vendor_details=[];
	...
	if (mysqli_num_rows($result) > 0) {
		while($row = mysqli_fetch_assoc($result)) {
			...
			$vendor_details[]=$row['company_name'];
			};
			
	}
	var_dump($vendor_details);
	echo implode('¤',$vendor_details);
}
?>